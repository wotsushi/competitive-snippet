...
@parameterized.expand(
    [
        ...
        for snippet_dir, snippet_name in map(os.path.split, glob.glob(f"snippets/*/*"))
        for serial in sorted(
            map(os.path.basename, glob.glob(f"{snippet_dir}/{snippet_name}/in/*"))
        )
    ]
)
def test_rust(name, snippet_path, snippet_name, serial):
    ...
    with open(f"{snippet_path}/in/{serial}") as input_test, open(
        f"{snippet_path}/out/{serial}"
    ) as ans_test:
